...
public class PCMazeGenerator
{
     ...
     * <param name="i">ligne de l'actuel tuile � cr�er/modifier</param>
     * <param name="j">colone de l'actuelle tuile � cr�er/modifier</param>
     * <param name="color">couleur en cour de g�n�ration</param>
     * 
    ...
    private int? GeneratePath(PCTile.PCFluidDirection oldDir, int i, int j)
    {
        ...
        while (result == null)
        {
            //On r�tabli la case comme elle �tait avant
            maze[i][j] = oldTile;
            possibles = GetPossibleDirections(oldDir, i, j);
            if (possibles.Count == 0)
            {
                //ttes les directions possibles ont �t� test�e et elle m�ne toute � un �chec
                return null;
            }
            //on choisit une nouvelle direction al�atoir parmis clle pas encore test�e
            nbAlea = Random.Range(0, possibles.Count);
            choosenOne = possibles[nbAlea];
            //on applique la dir sur la case
            //BUG: old-tile est modifi�
            maze[i][j].AddDirection(oldDir, choosenOne);
            newI = i;
            newJ = j;
            if (choosenOne == PCTile.PCFluidDirection.Down)
